...
#  0.15: 准确率51.75% / 平局Precision31.8% / 价值下注ROI(≥+0.08) -1.07%  ← 三项最优（与落盘权威口径对齐）
#  0.20: 准确率51.27% / 平局Precision31.5% / 价值下注ROI -1.82%
#  0.25: 准确率50.62% / 平局Precision31.0% / 价值下注ROI -2.39%
#  外样本边际平局命中率：0.15→0.20 为 30.8%，0.20→0.25 为 30.0%——低于样本内(33.7%/33.9%)且均低于35%质量线，
#  坐实"拉权重只能买到尾巴货"。draw_binary 独立通道外样本 Precision 最高仅 29.5%（≥0.40 档）、ROI 全负，
#  不可单独作平局下注信号（仅作融合的辅助权重）。0.15 终审通过，不再改动。
#  注意：样本内回填(55.52%)显著虚高于外样本(51.75%)，生产预期以 walk_forward 为准。】
#  【2026-08-09 复核对齐 + P0 重跑结案：以上外样本数字已与落盘权威口径 backtest/wf_fused_summary.txt（57维·全局Platt校准）逐项对齐。
#   P0 确定性重跑（seed42/57维/B365）于 2026-08-05 20:23 完成、全部落盘产物重新生成，0.15 精确复现 51.75%/31.8%/-1.07%，
#   幽灵数字 51.44%/+1.41% 确认不可复现、正式作废（+1.41% 与 C-016 价值腿移除矛盾）。
#   价值下注 ≥+0.08 外样本 ROI 三档全负（-1.07% / -1.82% / -2.39%），与 C-016 一致；0.15 终审通过，不再改动。】

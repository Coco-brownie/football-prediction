...
# 页面配置
st.set_page_config(page_title="策略回测 - 三AI风格对比", page_icon="🏆", layout="wide")
...
# ========== 页面主体 ==========
st.markdown("# 🏆 三AI策略回测")
st.markdown("赛季制 · 每赛季1000初始积分 · 凯利动态仓位 · 严格OOS验证")
...
with st.expander("📝 最近出手记录（含筛选）", expanded=False):
    ...
    def enrich_bet_df(log_df):
        ...
        df['仓位'] = (df['bet_amount'] / bet_before).apply(lambda x: f"{x:.1%}")
        # 格式化
        ...
        if len(log_df) > 0:
            ...
            if len(df) > 0:
                show = df[['match_date', 'ai_name', 'home_team', 'away_team', 'pred_short', 
                           'confidence_pct', 'odds', '仓位', '共识', '价值', 'actual_short', 'win_label']]
                show.columns = ['日期', 'AI', '主队', '客队', '预测', '置信度', '赔率', '仓位', '共识', '价值', '赛果', '结果']
            ...
            if len(log_df) > 0:
                ...
                if len(df) > 0:
                    show = df[['match_date', 'home_team', 'away_team', 'pred_short', 
                               'confidence_pct', 'odds', '仓位', '共识', '价值', 'actual_short', 'win_label']]
                    show.columns = ['日期', '主队', '客队', '预测', '置信度', '赔率', '仓位', '共识', '价值', '赛果', '结果']
                    st.dataframe(show, use_container_width=True, hide_index=True)
                    st.caption(f"共 {len(df)} 条记录 | 胜率: {(df['win'].mean()):.1%}")
                else:
                    st.info("当前筛选条件下无记录")
...
with st.expander("🦓 策略动物园 · 140组参数回测排行榜（点击展开）"):
    ...
    if len(zoo_exists) > 0:
        ...
        zoo_show['平均赔率'] = zoo_show['avg_odds'].apply(lambda x: f"{x:.2f}")
        ...
        zoo_show['封顶ROI'] = zoo_show['capped_roi'].apply(lambda x: f"{x:.0%}")
        zoo_show['固定ROI'] = zoo_show['fixed_roi'].apply(lambda x: f"{x:.0%}")

        display_cols = ['排名', '置信度', '凯利系数', '安全边际', '出手/赛季', '胜率', '最大回撤', '夏普', '卡尔玛', '封顶ROI', '固定ROI']
...
with st.expander("📖 策略规则说明"):
    ...
    这是理论回测的数学结果，现实中会因滑点、赔率变动、仓位上限等因素大幅缩水。
    **相对排名和策略特性才是有价值的参考**。
    ...
    **2. 实战可靠性验证**
    - 滑点测试：赔率打9折后ROI衰减~25%，全部策略仍正收益，Alpha有厚度

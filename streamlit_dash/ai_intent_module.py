...
#   【2026-08-18 仓位收紧】kelly_fraction 仅作排序参考；历史 MC 复核显示固定 1%~2% 较安全（研究侧，非建议）：
...
# 价值策略（研究保留 · 独立出手逻辑）：EV = 模型概率 × 赔率 - 1
# 研究：EV≥0.10 全期 12416 场 +3.01%、2012 后 7701 场 +8.97%（去重后现行口径，旧 9050/+9.53% 作废）；
# C-015 无后见之明验证不成立 + C-016 已从产品化组合移除 → 仅研究观察，不作为组合腿
...
# ==================== 产品分级体系（研究→产品映射） ====================
# 对齐金标准《统一策略金标准验证结果》：样本内成立 ≠ 可直接采用（需 MC 资金曲线安全 + 独立期复现）
# 等级：research=🔬研究参考 / beta=⚠️实验性(Beta) / watch=❌仅观察
...
PRODUCT_GRADING = {
    "三AI参考": {
        "level": "research",
        "label": "🔬 研究参考",
        "note": "三AI 为研究性输出；历史回测数字不代表未来收益，不构成任何建议",
    },
    "猎鹰Plus·基础版": {
        "level": "research",
        "label": "🔬 研究参考（待复现）",
        "note": "=S1 冷门猎手（样本内 1759场 +15.79%），真实可成交赔率口径（Pinnacle 终盘）未过 Bonferroni，待 2026-2027 独立期复现",
    },
    "猎鹰Plus·德甲专精": {
        "level": "beta",
        "label": "⚠️ 实验性(Beta)",
